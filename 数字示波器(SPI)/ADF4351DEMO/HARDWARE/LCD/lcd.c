...
void LCD_Showcoordinatesystem(u16 xbasepoint,u16 ybasepoint,u16 xstep,u16 ystep,u16 steplength)
{
	...
	LCD_DrawLine(xbasepoint+steplength*xstep-10, ybasepoint-7, xbasepoint+steplength*xstep, ybasepoint); //arrow X
	LCD_DrawLine(xbasepoint+7, ybasepoint-steplength*ystep+10, xbasepoint, ybasepoint-steplength*ystep); //axis Y
	LCD_DrawLine(xbasepoint-7, ybasepoint-steplength*ystep+10, xbasepoint, ybasepoint-steplength*ystep); //axis Y
	LCD_ShowNum(xbasepoint-10,ybasepoint+10,0,1,16);  //写零点
	LCD_ShowString(xbasepoint+steplength*xstep, ybasepoint+10,32,32,16,(u8*)"x"); //写x
  LCD_ShowString(xbasepoint-15, ybasepoint-steplength*ystep,32,32,16,(u8*)"y"); //写y
	
	for(i=1;i<xstep;i++)
	{
		LCD_DrawLine(xbasepoint+steplength*i,ybasepoint+5,xbasepoint+steplength*i,ybasepoint-5);//画横轴上的点
    LCD_ShowxNum(xbasepoint+steplength*i-8,ybasepoint+12,80.0+4.0*i,3,16,0);
	}		
	
	for(i=1;i<ystep;i++)
	{
		LCD_DrawLine(xbasepoint,ybasepoint-steplength*i,xbasepoint+5,ybasepoint-steplength*i);//画横轴上的点
	}		
...
{
	return ((R*0x1F/0xFF)<<11)+((G*0x3F/0xFF)<<5)+(B*0x1F/0xFF);
}
